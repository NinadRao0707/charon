...
| **MCP06 — Intent Flow Subversion / confused deputy** | Phase 5 implements RFC 8693 delegation with `act`-claim chains: the originating human is preserved as `sub` and every actor is recorded, so any multi-hop action is traceable back to the authorizing human and delegated authority can only narrow. This constrains confused-deputy abuse and gives non-repudiation, though it does not by itself stop prompt injection. | 5 ✅ (partial) |
